...
class TurtleControllerNode(Node):
    ...
    def call_set_pen_service(self,r,g,b,width,off):
        client = self.create_client(SetPen, "/turtle1/set_pen") #create client with service type and service name

        # wait for service to become available
        while not client.wait_for_service(1.0):
            self.get_logger().warn("waiting for service...")

        # once service is available, create request and set its parameters
    ...
    def callback_set_pen(sefl,future):
        try:
